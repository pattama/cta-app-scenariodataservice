...
class ScenariosHandler extends RestCRUD {
  ...
  run(req, res) {
    const user = {};
    const uid = req.params.uid || req.headers.uid;

    if (uid === this.cementHelper.cement.configuration.properties.uid) {
      user.uid = uid;
    } else {
      user.id = req.params.user_id || req.headers.user_id;
      user.first = req.params.user_first || req.headers.user_first;
      user.last = req.params.user_last || req.headers.user_last;
      user.uid = uid;
    }
