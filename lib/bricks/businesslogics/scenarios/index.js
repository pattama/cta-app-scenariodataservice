...
class Scenarios extends Base {
  constructor(cementHelper, configuration) {
    ...
    this.apiURLs.schedulerApiUrl =
      this.cementHelper.cement.configuration.properties.schedulerApiUrl || configuration.properties && configuration.properties.schedulerApiUrl;
    this.apiURLs.scenarioApiUrl =
      this.cementHelper.cement.configuration.properties.scenarioApiUrl || configuration.properties && configuration.properties.scenarioApiUrl;
  ...
  start() {
    ...
    return this.setupSchedules().then((schedules) => {
      this.logger.info(`Register [${schedules.length}] scenarios to scheduler`);
    }).catch((err) => {
