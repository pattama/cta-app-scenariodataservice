 ...
 */
class InsertOne extends BaseMongoDBHelper {
  ...
  _process(context) {
    ...
    const mongoDbDocument = new that.schemas[mongoDbCollection](context.data.payload.content);
    ...
    output.on('done', function(brickname, response) {
      if (response.hasOwnProperty('result') && response.hasOwnProperty('ops')) {
        if (response.result.ok && response.result.n > 0 && response.ops.length > 0) {
          const result = that.schemas[mongoDbCollection].toCTAData(response.ops[0]);
