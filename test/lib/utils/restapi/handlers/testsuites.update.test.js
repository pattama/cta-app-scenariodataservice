...
const TESTSUITE = require('./testsuites.update.test.data.json');

describe('Utils - RESTAPI - Handlers - testsuites - update', () => {
  let handler;
  before(() => {
  ...
  context('when id is provided (update)', () => {
    ...
    context('when document is not found', () => {
      ...
      it('should send 404', () => {
        ...
        sinon.assert.calledWith(res.send, `testsuite '${req.params.id}' not found.`);
      });
    });
  });
});
